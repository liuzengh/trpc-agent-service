...
当前已经有可运行的平台代码，不只是题目设计。日常操作从[本地运行与升级手册](docs/operations-runbook.md)开始；开发进度看[执行清单](docs/execution-plan.md)，完成范围看[功能状态](docs/feature-status.md)。下文保留原始需求，便于对照验收。
...
- [只读项目文档 MCP](docs/project-docs-mcp.md)
...
仓库中的“已编码”“自动测试”“本地集成”和“真实联调”是不同状态。Telegram 与企业微信**消息 MCP 群文本**已完成开发环境真实 Agent 基础收发；企业微信**自建应用回调**仍只有代码和模拟协议测试。真实群文本成功不代表媒体、生产容量或 Kubernetes 已验收。各项能力的具体边界以[功能实现与验证状态](docs/feature-status.md)为准。
