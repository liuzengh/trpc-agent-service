...
实际代码组织（后端仅 `trpcservice/` 单包族 + `cmd/trpc-service` 入口；前端在 `front/`，部署资产在 `deployments/`）。模块边界对齐设计方案，但不再按示范目录创建子包。
...
`-- trpcservice            # 后端源码
    |-- agent              # Agent 领域（Manager 内嵌内存态 + MySQL 持久化双实现）
    |-- audit              # 治理决策与请求记账（MySQL 异步批量 + 读侧）
    |-- bus                # Redis Streams + 路由/幂等/会话锁
    |-- channels/{wecom,feishu}   # IM 适配器（统一契约 + 平台实现）
    |-- config / health / log      # 配置/健康/日志
    |-- knowledge          # 知识库（Milvus 2.5+ / 内存双向量库 + 框架 RAG 管线）
    |-- llm                # ModelEndpoint 多协议（OpenAI/Anthropic/Gemini）
    |-- metrics            # 平台 OTel 指标（复用框架 MeterProvider）
    |-- skill              # Skill 四级资产（Skill/Version 内存态内嵌 + MySQL 双实现）
    |-- storage            # Session/Memory 多后端 + MinIO artifact（S3 兼容）
    |-- tenant / tool             # 租户/工具
    |-- web                # Admin REST（tenants/endpoints/agents/tools/kbs/skills/audit）
    |-- worker             # inbound → runner.Run → outbox（幂等 + 会话锁 + 审批 + Skill/artifact 注入 + 审计）
    `-- workspace          # Docker 代码执行（code-exec：隔离容器 + 无网络 + 高风险审批）
```

> 能力边界：`workspace` 承载 Docker 代码执行（code-exec，隔离容器+无网络+高风险自动审批；K8s Pod 后端已决策不需要——生产部署为 Docker Compose）；Admin 对话（/chat，SSE）与 IM 通道绑定（/channels CRUD）已实现；`chat_messages` 账本表已启用（会话历史）；secret manager 已实现（统一凭据 AES-256-GCM，见 ADR-0002）；IM 桥接层（gateway，adapter↔总线双向）已实现——企业微信/飞书真实 WSS Conn 与 main 启动监听仍需本地账号手测接线（适配器与单测已就绪）。
