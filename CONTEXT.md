...
- **治理策略（Tenant Governance Policy）**：租户级治理配置（`tenants.quota.token_quota` + `tenants.audit_policy` JSON），由 worker `resolveTenantPolicy` **每消息**解析为快照并随 ctx 传递（`app/worker/policy.go`），保证 handle（IM 白名单）与 run（预算/工具白名单/审批并集/脱敏）决策一致。四类策略：**脱敏（Redact）**——默认开启，正则屏蔽工具参数/结果中的身份证/银行卡/手机号/邮箱（`governance.Redact` 框架 BeforeTool/AfterTool plugin），租户可显式关；**预算（Budget）**——`quota.token_quota > 0` 才生效（**未配置=不限**），模型调用前从 usage_records 累计量拦截；**工具白名单（ToolWhitelist）**——agent 挂载工具须∈租户白名单才生效（knowledge_search 不受约束）；**IM 用户白名单（IMAllowUsers）**——空=全放行，只对 IM 通道(wecom/feishu)生效、admin 通道豁免。**强制审批（ForceApproval）**：租户强制审批工具集，与 agent `approval_tool_ids` ∪ `risk_level=high` 并集。治理失败不硬失败消息（回落安全默认并告警）。与审批互补：审批管"高风险调用是否放行"，治理管"谁能用、花多少、数据怎么脱敏"。
...
## 分层架构（阶段 32 确立）
- **分层目录**：`trpcservice/` 分 `domain/`（领域层：模型 + 导出 `Store` 接口 + 内存默认，纯逻辑零 IO）、`infra/`（基础设施层：storage/bus/audit/secret/metrics/workspace/channels/config/log/health 的实现）、`app/`（应用层：worker 编排 + web 入口）。依赖方向 `domain ← infra ← app`，禁止反向。
- **域 Store 子包**：各域 MySQL 等持久化实现集中 `infra/storage/<域>store`（tenantstore/agentstore/llmstore/toolstore/skillstore/knowledgestore/bindingstore/ledgerstore），实现域包导出的 `Store` 接口；共享 SQL helper 在 `storage/sqlutil`。域 Manager 通过 `NewManagerWithStore`/`NewRegistryWithStore` 注入存储实现，内存默认由 `NewManager`/`NewRegistry` 提供。
