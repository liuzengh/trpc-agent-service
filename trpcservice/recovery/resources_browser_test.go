...
(async () => {
  ...
  try {
    ...
    await page.getByRole('alert').filter({hasText:'知识库已支持网页配置和文本资料管理'}).waitFor();
    await page.getByRole('button',{name:'上传资料',exact:true}).click();
    const knowledgeDialog=page.getByRole('dialog');
    await knowledgeDialog.getByLabel('资料名称',{exact:true}).fill('Browser handbook');
    await knowledgeDialog.locator('input[type=file]').setInputFiles({name:'browser.md',mimeType:'text/markdown',buffer:Buffer.from('# Browser handbook\nRetention is forty days.\n')});
    const knowledgeUploaded=page.waitForResponse(r=>r.url().endsWith('/admin/knowledge/documents'));
    await knowledgeDialog.getByRole('button',{name:'提交入库',exact:true}).click();
    if((await knowledgeUploaded).status()!==202)throw new Error('knowledge upload did not enqueue');
    await page.getByRole('row').filter({hasText:'Browser handbook'}).waitFor();
    ...
    if((await knowledgeToggle.getAttribute('aria-checked'))!=='true')await knowledgeToggle.click();
    const provider=page.locator('.ant-form-item').filter({hasText:'Embedding 供应商'}).getByRole('combobox');
    await provider.click();
    await page.locator('.ant-select-dropdown:visible .ant-select-item-option-content').filter({hasText:'OpenAI-compatible Embedding'}).click();
    await page.locator('.ant-form-item').filter({hasText:'Embedding 模型'}).getByRole('textbox').fill('browser-embedding');
    await page.locator('.ant-form-item').filter({hasText:'Embedding API 地址'}).getByRole('textbox').fill('https://embedding.example.invalid/v1');
    await page.locator('.ant-form-item').filter({hasText:'Embedding API Key'}).getByRole('textbox').fill('browser-embedding-secret');
    const credentialSaved=page.waitForResponse(r=>r.url().endsWith('/admin/knowledge/embedding-credential'));
    const knowledgeDraftSaved=page.waitForResponse(r=>r.url().endsWith('/admin/drafts/save'));
    await page.getByRole('button',{name:/保存草稿/}).click();
    const credentialResponse=await credentialSaved;
    if(credentialResponse.status()!==201||(await credentialResponse.text()).includes('browser-embedding-secret'))throw new Error('embedding credential was not stored safely');
    const knowledgeDraft=await knowledgeDraftSaved;
    if(knowledgeDraft.status()!==200)throw new Error('knowledge draft save failed: '+await knowledgeDraft.text());
    const knowledgeConfig=knowledgeDraft.request().postDataJSON().config.knowledge_config;
    if(!knowledgeConfig.embedding.secret_ref.startsWith('managed://')||JSON.stringify(knowledgeConfig).includes('browser-embedding-secret'))throw new Error('knowledge draft did not use encrypted credential reference');
    if(errors.length)throw new Error(errors.join('\n'));
    console.log('Browser passed: knowledge upload, encrypted Embedding setup, storage credentials, Skill approval and Agent selection.');
