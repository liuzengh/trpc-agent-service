...
func (r *TenantRegistry) Tenant(ctx context.Context, id string) (tenant.Tenant, error) {
	...
	var backend []byte
	err = r.pool.QueryRow(queryCtx, `SELECT tenant_id, name, status, config_version, default_agent_app_id, backend_config FROM tenant WHERE tenant_id=$1`, id).Scan(&value.ID, &value.Name, &value.Status, &value.ConfigVersion, &defaultAgent, &backend)
	if err != nil {
...
func (r *TenantRegistry) ResolveBinding(ctx context.Context, channel, externalAppID string) (tenant.ChannelBinding, error) {
	...
	defer cancel()
	rows, err := r.pool.Query(queryCtx, `SELECT tenant_id, channel, binding_id, external_app_id, secret_ref, verify_token_ref, status, enabled, version, created_at, updated_at, expires_at, disabled_at, external_target_type, external_target_id FROM channel_binding WHERE channel=$1 AND external_app_id=$2`, channel, externalAppID)
	if err != nil {
...
func (r *TenantRegistry) CreateBinding(ctx context.Context, tc tenant.TenantContext, value tenant.ChannelBinding) error {
	...
	defer cancel()
	_, err = r.pool.Exec(queryCtx, `INSERT INTO channel_binding (tenant_id, channel, binding_id, external_app_id, secret_ref, verify_token_ref, status, enabled, version, expires_at, disabled_at, external_target_type, external_target_id) VALUES ($1,$2,$3,$4,NULLIF($5,''),NULLIF($6,''),$7,$8,$9,$10,$11,$12,NULLIF($13,''))`, value.TenantID, value.Channel, value.ID, value.ExternalAppID, value.SecretRef, value.VerifyTokenRef, value.Status, value.Enabled, value.Version, nullableTime(value.ExpiresAt), nullableTime(value.DisabledAt), value.ExternalTargetType, value.ExternalTargetID)
