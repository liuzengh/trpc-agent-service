...
func (s *PostgresStore) Checkpoint(ctx context.Context, key PollKey, hash string, start time.Time) (Checkpoint, error) {
	_, err := s.db.ExecContext(ctx, `INSERT INTO channel_poll_checkpoint(tenant_id,channel_binding_id,chat_hash,config_hash,through_at) VALUES($1,$2,$3,$4,$5) ON CONFLICT DO NOTHING`, key.TenantID, key.BindingID, key.ChatHash, hash, start)
	if err != nil {
		return Checkpoint{}, errors.New("cannot initialize channel checkpoint")
	}
	var value Checkpoint
	err = s.db.QueryRowContext(ctx, `SELECT config_hash,through_at,version FROM channel_poll_checkpoint WHERE tenant_id=$1 AND channel_binding_id=$2 AND chat_hash=$3`, key.TenantID, key.BindingID, key.ChatHash).Scan(&value.ConfigHash, &value.Through, &value.Version)
	if err != nil {
...
func (s *PostgresStore) Advance(ctx context.Context, key PollKey, previous Checkpoint, through time.Time) error {
	result, err := s.db.ExecContext(ctx, `UPDATE channel_poll_checkpoint SET through_at=$6,version=version+1,updated_at=now() WHERE tenant_id=$1 AND channel_binding_id=$2 AND chat_hash=$3 AND config_hash=$4 AND version=$5 AND through_at<=$6`, key.TenantID, key.BindingID, key.ChatHash, previous.ConfigHash, previous.Version, through)
...
func (s *PostgresStore) Ready(ctx context.Context) error {
	var ok bool
	if err := s.db.QueryRowContext(ctx, `SELECT to_regclass('channel_poll_checkpoint') IS NOT NULL AND to_regclass('channel_poll_seen') IS NOT NULL AND to_regclass('channel_delivery_attempt') IS NOT NULL`).Scan(&ok); err != nil || !ok {
