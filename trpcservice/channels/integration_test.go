...
func TestMySQLBindingStoreLifecycle(t *testing.T) {
	...
	if err := s.Create(ctx, ChannelBinding{
		...
		Channel: ChannelWeCom, AccountID: "wx-1", CredentialRef: "sec://wx-1",
	}); err != nil {
	...
	if err := s.Create(ctx, ChannelBinding{
		...
		t.Errorf("dup err = %v, want ErrBindingDuplicate", err)
	}
	if err := s.Create(ctx, ChannelBinding{
	...
	if got.CredentialRef != "sec://wx-1" || got.Channel != ChannelWeCom {
		t.Errorf("get = %+v", got)
	}
	...
	wecom, _ := s.List(ctx, "", ChannelWeCom)
	...
	if err := s.Delete(ctx, "b-missing"); !errors.Is(err, ErrBindingNotFound) {
