...
// message_id.
func (l *MySQLLedger) RecordTurn(ctx context.Context, t Turn) error {
	...
	if _, err := tx.ExecContext(ctx, `
		...
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'SUCCESS')`,
		t.UserMsgID, t.SessionID, t.AgentID, t.MemberID, t.TurnID, t.TurnTS, RoleUser, t.UserText); err != nil {
	...
	if t.ReplyMsgID != "" {
		if _, err := tx.ExecContext(ctx, `
			INSERT IGNORE INTO chat_messages
				(message_id, session_id, agent_id, member_id, turn_id, turn_timestamp, role, content, status)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'SUCCESS')`,
			t.ReplyMsgID, t.SessionID, t.AgentID, t.MemberID, t.TurnID, t.TurnTS, RoleAssistant, t.ReplyText); err != nil {
...
func (l *MySQLLedger) Sessions(ctx context.Context, q SessionQuery) ([]Session, error) {
	limit := q.Limit
	if limit <= 0 {
		limit = 20
	}
...
func (l *MySQLLedger) Messages(ctx context.Context, q MessageQuery) ([]Message, error) {
	limit := q.Limit
	if limit <= 0 {
		limit = 50
	}
