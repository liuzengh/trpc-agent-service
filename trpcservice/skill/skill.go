// Package skill implements the four-level skill model (Skill / SkillVersion /
// SkillScript / SkillReference) and the M:N agent binding with a version lock.
// A skill is stored as rows and loaded into the model context as text at
// runtime; the storage form is invisible to the model.
//
// Scripts and references are kept in MySQL for completeness (they are part of
// the SKILL.md bundle), but the runtime currently consumes only ContentMD and
// PromptTemplate from SkillVersion. Executing SkillScript with the K8s code
// executor is future work.
package skill
...
// concurrent use; the embedded memStore uses a single mutex. NewMySQLManager
// switches persistence to MySQL using the same Manager API.
type Manager struct {
	store store
}
...
// deployments/mysql/init/005_skills.sql and is applied by the MySQL container
// init scripts.
func NewMySQLManager(db *sql.DB) *Manager {
	return &Manager{store: newMySQLStore(db)}
}
