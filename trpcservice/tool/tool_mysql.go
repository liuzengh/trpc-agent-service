...
func NewMySQLRegistry(db *sql.DB) *Registry {
	return &Registry{store: &mysqlStore{db: db}}
}

// toolColumns lists the columns shared by Get and List.
const toolColumns = "tool_id, scope, tenant_id, name, definition, risk_level"

// Register upserts a tool definition; re-registering a soft-deleted tool
// revives it (is_deleted=0), matching the in-memory replace semantics.
func (s *mysqlStore) Register(ctx context.Context, d Definition) error {
...
func (s *mysqlStore) Get(ctx context.Context, id string) (Definition, error) {
	row := s.db.QueryRowContext(ctx,
		"SELECT "+toolColumns+" FROM tools WHERE tool_id = ? AND is_deleted = 0", id)
	return scanTool(row)
}

func (s *mysqlStore) List(ctx context.Context, tenantID string) ([]Definition, error) {
...
// Grant whitelists a tool for an agent; INSERT IGNORE keeps it idempotent.
...
// Revoke drops an agent's grant; deleting a missing grant is a no-op, like
// the in-memory store.
...
// scanTool materializes a row; the definition JSON is authoritative for
// name/description, the name column is its indexed copy for the unique key.
func scanTool(rs rowScanner) (Definition, error) {
	var (
		d        Definition
