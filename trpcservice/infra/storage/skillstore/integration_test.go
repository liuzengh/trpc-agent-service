...
func TestMySQLSkillLifecycle(t *testing.T) {
	...
	// Create -> defaults
	acme := "acme"
	s := &skill.Skill{Code: "mysql-skill", Name: "MySQL skill.Skill", Scope: skill.ScopeTenant, OwnerTenantID: &acme}
	if err := m.Create(ctx, s); err != nil {
		t.Fatalf("create: %v", err)
	}
	if s.SkillID == "" || s.Status != skill.StatusDraft || s.CurrentVersion != 0 {
		t.Errorf("defaults: %+v", s)
	}

	// Duplicate code rejected by the unique index.
	dup := &skill.Skill{Code: "mysql-skill", Name: "Dup", Scope: skill.ScopeTenant, OwnerTenantID: &acme}
	if err := m.Create(ctx, dup); !errors.Is(err, skill.ErrSkillCodeExists) {
		t.Errorf("dup code = %v, want skill.ErrSkillCodeExists", err)
	}

	// Version round-trip + publish switches the pointer.
	if err := m.CreateVersion(ctx, &skill.SkillVersion{SkillID: s.SkillID, Version: 1, ContentMD: "# SKILL\nfirst"}); err != nil {
	...
	if cur.CurrentVersion != 1 || cur.Status != skill.StatusPublished {
	...
	if v1.Checksum == "" || v1.Status != skill.StatusPublished || v1.PublishedAt.IsZero() {
	...
	if err := m.Create(ctx, &skill.Skill{Code: "g-skill", Name: "G", Scope: skill.ScopeGlobal}); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := m.Get(ctx, s.SkillID); !errors.Is(err, skill.ErrSkillNotFound) {
