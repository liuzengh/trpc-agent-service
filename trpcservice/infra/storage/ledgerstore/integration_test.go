...
func TestMySQLLedgerRecordAndHistory(t *testing.T) {
	...
	sessions, err := ledger.Sessions(ctx, chat.SessionQuery{TenantID: "t1"})
	if err != nil {
	...
	msgs, err := ledger.Messages(ctx, chat.MessageQuery{SessionID: "s1"})
	if err != nil {
	...
	if msgs[1].Role != chat.RoleAssistant || msgs[1].Content != "hi there" {
	...
	if msgs, _ = ledger.Messages(ctx, chat.MessageQuery{SessionID: "s1"}); len(msgs) != 2 {
		t.Errorf("after redelivery messages = %d, want still 2", len(msgs))
	}

	// a second turn
	if err := ledger.RecordTurn(ctx, chat.Turn{
		TenantID: "t1", AgentID: "a1", SessionID: "s1", MemberID: "u1", Channel: "admin",
	...
	all, _ := ledger.Messages(ctx, chat.MessageQuery{SessionID: "s1"})
	if len(all) != 4 {
		t.Fatalf("after turn2 messages = %d, want 4", len(all))
	}
	// paginate: only the older turn (turn_timestamp < 2000)
	older, _ := ledger.Messages(ctx, chat.MessageQuery{SessionID: "s1", BeforeTurn: 2000})
	...
	otherSessions, _ := ledger.Sessions(ctx, chat.SessionQuery{TenantID: "t-other"})
	...
	sess, _ := ledger.Sessions(ctx, chat.SessionQuery{TenantID: "t1", MemberID: "u2"})
