...
func (s *mysqlStore) Update(ctx context.Context, ep llm.Endpoint) error {
	...
	res, err := s.db.ExecContext(ctx,
		...
		ep.Scope, sqlutil.Null(ep.TenantID), ep.Name, llm.NormalizeProvider(ep.Provider), normalizeType(ep.Type), ep.BaseURL, ep.ModelName, ep.APIKeyRef, ep.ID)
	if err != nil {
		return err
	}
...
func scanEndpoint(sc sqlutil.RowScanner) (llm.Endpoint, error) {
	...
	if err := sc.Scan(&ep.ID, &ep.Scope, &tenantID, &ep.Name, &ep.Provider, &ep.Type, &ep.BaseURL, &ep.ModelName, &ep.APIKeyRef); err != nil {
