...
func (r *MySQLRecorder) Record(e Entry) {
	...
	case r.ch <- e:
	default:
		// Buffer full: audit is best-effort, but a dropped entry is a lost
		// audit record, so make it loud (the counter is also exposed via
		// Dropped for tests/ops).
		slog.Warn("audit: buffer full, dropping entry", "dropped", atomic.AddInt64(&r.dropped, 1))
	}
}
