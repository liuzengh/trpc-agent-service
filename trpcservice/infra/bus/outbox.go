...
func (o *Outbox) dispatchBatch(ctx context.Context, b Bus) (int, error) {
	...
	for _, e := range batch {
		...
		if err := json.Unmarshal([]byte(e.payload), &m); err != nil {
			// Poison payload: it can never publish. Record and move on; a
			...
			if _, err := o.db.ExecContext(ctx,
				...
				slog.Warn("bus: mark poison event sent failed", "event", e.id, "err", err)
			}
			continue
		}
		if err := b.PublishOutbound(ctx, &m); err != nil {
			if _, err := o.db.ExecContext(ctx,
				`UPDATE outbox_events SET retries = retries + 1 WHERE event_id = ?`, e.id); err != nil {
				slog.Warn("bus: outbox retry bump failed", "event", e.id, "err", err)
			}
			continue
		}
		if _, err := o.db.ExecContext(ctx,
