...
// TestLoadExpandsEnvVars verifies ${ENV} references are expanded before YAML
// parsing (the mechanism that keeps the MySQL DSN out of version control).
func TestLoadExpandsEnvVars(t *testing.T) {
	t.Setenv("TRPC_TEST_DSN", "secret@tcp(db:3306)/app")
	dir := t.TempDir()
	path := filepath.Join(dir, "config.yaml")
	content := "server:\n  http_addr: \":8080\"\n" +
		"mysql:\n  dsn: \"${TRPC_TEST_DSN}?parseTime=true\"\n"
	if err := os.WriteFile(path, []byte(content), 0o644); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}
	...
	if cfg.MySQL.DSN != "secret@tcp(db:3306)/app?parseTime=true" {
		t.Errorf("MySQL.DSN = %q, want env-expanded dsn", cfg.MySQL.DSN)
	}
}

// TestLoadUndefinedEnvExpandsEmpty verifies an unset variable expands to the
// empty string (memory mode), not an error.
func TestLoadUndefinedEnvExpandsEmpty(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "config.yaml")
	content := "server:\n  http_addr: \":8080\"\nmysql:\n  dsn: \"${TRPC_DEFINITELY_UNSET_DSN}\"\n"
	if err := os.WriteFile(path, []byte(content), 0o644); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}
	cfg, err := Load(path)
	...
	if cfg.MySQL.DSN != "" {
		t.Errorf("MySQL.DSN = %q, want empty for unset env", cfg.MySQL.DSN)
	}
}
