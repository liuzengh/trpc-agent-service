...
func Init() {
	...
	modelCallDuration, _ = meter.Float64Histogram(
...
func ModelCallDuration(ctx context.Context, tenantID, agentID string, dur time.Duration) {
	modelCallDuration.Record(ctx, dur.Seconds(), otelmetric.WithAttributes(attrTenant.String(tenantID), attrAgent.String(agentID)))
}
