...
func (s *Service) TransitionBackendMigration(
	...
	if (current.ResourceType == "knowledge" || current.ResourceType == "session" || current.ResourceType == "memory") && len(verification) > 0 {
		return controlplane.BackendMigration{}, invalidf("migration verification is generated by server jobs, not supplied by callers")
	}
	if len(checkpoint) > 0 {
