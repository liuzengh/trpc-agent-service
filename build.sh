...
# Package only committed source; normal builds never load .env or start services.
package_source() {
  PACKAGE_ROOT="$(cd "$(dirname "$0")" && pwd)"
  cd "$PACKAGE_ROOT"
  umask 077
  [[ $# == 0 ]] || { echo "usage: ./build.sh --package" >&2; exit 2; }
...
mkdir -p "$ROOT/bin"
COMMANDS=(trpc-service trpc-local trpc-migrate trpc-init trpc-modelcheck trpc-embeddingcheck trpc-wecomcheck trpc-wecomsetup trpc-permissions)
