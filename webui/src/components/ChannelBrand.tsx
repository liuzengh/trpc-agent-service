...
export function ChannelBrandIcon({ channel, size = 18, className }: { channel: string; size?: number; className?: string }) {
  if (channel === 'telegram') {
    return (
  ...
  if (channel === 'wecom') {
    return <img aria-hidden="true" alt="" className={className} width={size} height={size} src={`${import.meta.env.BASE_URL}brands/wecom.png`} draggable={false} />
  }

  if (channel === 'feishu') {
    return <img aria-hidden="true" alt="" className={className} width={size} height={size} src={`${import.meta.env.BASE_URL}brands/feishu.png`} draggable={false} />
  }
