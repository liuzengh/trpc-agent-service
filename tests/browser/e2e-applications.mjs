...
try {
  ...
  assert.equal(await supportDialog.getByRole('checkbox', { name: '启用 查询订单与物流信息' }).isChecked(), true)
  assert.equal(await supportDialog.getByRole('checkbox', { name: '启用 发起订单退款' }).isChecked(), true)
  assert.equal(await supportDialog.getByRole('checkbox', { name: '启用 向用户发放优惠券' }).isChecked(), false)
  assert.equal(await supportDialog.getByRole('checkbox', { name: '发起订单退款 执行前审批' }).isChecked(), true)
  assert.equal(await supportDialog.getByRole('checkbox', { name: '查询订单与物流信息 执行前审批' }).isChecked(), false)
  ...
  assert.equal(await effortSelect.isDisabled(), true, 'reasoning effort must be disabled before reasoning mode is enabled')
  await supportDialog.locator('label').filter({ hasText: '启用推理模式' }).locator('input[type="checkbox"]').check()
  assert.equal(await effortSelect.isDisabled(), false)
  ...
  assert.equal(await dialog.getByRole('checkbox', { name: '启用 查询订单与物流信息' }).isChecked(), false, 'create dialog should use the same tool catalog with no default grants')
