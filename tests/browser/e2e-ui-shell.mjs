...
try {
  ...
  const botSettings = page.getByRole('button', { name: '机器人设置' })
  await botSettings.waitFor({ timeout: 3000 })
  ...
  await botSettings.click()
  await page.getByRole('button', { name: '创建机器人', exact: true }).waitFor({ timeout: 3000 })
  assert.match(page.url(), /[?&]tab=bots(?:&|$)/, 'robot settings must open the hidden robot-management destination')
  assert.equal(await primaryNavigation.getByText('机器人', { exact: true }).count(), 0, 'robot management must remain absent from primary navigation after opening settings')
  await page.getByRole('tab', { name: '对话', exact: true }).click()
  assert.match(page.url(), /[?&]tab=chat(?:&|$)/, 'primary navigation must keep the URL synchronized')
  ...
  assert.equal(await page.locator('.login-callback-panel').getByText('https://console.example.com/api/v1/auth/callback', { exact: true }).count(), 1)
  ...
  await page.getByRole('button', { name: '使用 Mock 登录', exact: true }).waitFor()
  assert.equal(await page.getByRole('button', { name: /飞书/ }).count(), 0, 'disabled enterprise providers must stay off the public login page')
  assert.equal(await page.getByRole('button', { name: /企业 SSO/ }).count(), 0, 'disabled enterprise providers must stay off the public login page')
  ...
  assert.equal(await page.locator('.login-enterprise-provider').count(), 1, 'public login should show only enabled enterprise providers')
  assert.equal(await page.getByText(/trpc-agent-service/).count(), 0, 'login copy should omit implementation detail')
  assert.match(await page.locator('.login-brand').innerText(), /登录到工作区/)
