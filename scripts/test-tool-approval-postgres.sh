...
name="tool-approval-postgres-$RANDOM-$$"
...
ready=0
for _ in $(seq 1 90);do if docker exec "$name" psql -U postgres -d agent_platform -Atqc 'SELECT 1' 2>/dev/null|grep -qx 1;then ready=1;break;fi;sleep 1;done
...
SQL
export WORKER_APPROVAL_TEST_MIGRATION_URL="postgres://worker_migrator:fixture-migrator@127.0.0.1:$port/agent_platform?sslmode=disable"
export WORKER_APPROVAL_TEST_RUNTIME_URL="postgres://worker_runtime:fixture-runtime@127.0.0.1:$port/agent_platform?sslmode=disable"
cd "$root"
GOCACHE=${GOCACHE:-/tmp/tool-approval-gocache} go test -race ./services/agent-worker/internal/toolapproval -run '^TestApprovalPostgresLifecycle$' -count=1 -v
printf 'DANGEROUS_TOOL_APPROVAL_POSTGRES=PASS\n'
