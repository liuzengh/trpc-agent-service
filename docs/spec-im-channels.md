...
type Adapter interface {
    Type() Type
    // Callback 验签并解析一次 webhook；成功路径必须自行写协议级 ACK
    // （企微 "success"、echostr 探测等）；返回 nil,nil 表示探测已应答。
...
### 3.3 微信客服（挂起：按 9/3 决定推后，作为最后一个 IM 项补充）

- 验签 `signature=SHA1(sort(token,timestamp,nonce))`；事件通知后主动 `sync_msg` 拉取；`send_msg` 回复；
- 48h 会话窗：异步处理超时落库待补发/转人工（方案文档 3.3 已述）；
- 单测同 3.2 策略（向量 + mock HTTP）。
...
| 9/3 | 企微 Adapter + 官方向量单测；Admin API 切片 2（租户 CRUD/通道绑定/原子持久化/Registry 热更新）提前完成 | 企微代码路径可测；管理链路可用 |
| 9/4–9/5 | 路由串行化占位（单节点）；数据层评估（Redis session 换 inmemory） | 会话并发安全 |
| 9/8 前 | 微信客服 Adapter + 单测（挂起后补充，最后一个 IM 项） | 两种 IM 代码完成 |
| 9/9 | go.mod 与接口冻结 | 进入联调 |

## 6. 验收映射

- 原题「至少两种 IM」：企微 + 微信客服**均有代码实现与单测**；企微真实验证由导师代填，微信客服以 mock 验证，均符合导师 8/21 口径；微信客服实现按 9/3 决定挂起后置，9/8 前补齐；
