...
`backend_connection` 保存具名连接，真实密码、API Key 和 S3 访问密钥复用加密凭据库。外部目的地及其凭据仅由 superadmin 创建；租户管理员可选择所属租户的连接并建立绑定，或创建 InMemory 连接。连接接口不返回内部引用或秘密值，保存和审计处于同一事务。

托管凭据与已登记的资源类型、后端类型和配置绑定校验。旧绑定接口不能改写地址后复用同一凭据；租户管理员也不能绕过具名连接接口直接登记外部目的地。环境凭据仍由部署者按用途授权，可由平台管理员从已有引用列表选择。

连接配置不可原地覆盖。新增连接不会探测、建表或修改现有数据，绑定冲突不会自动替换旧后端；有数据的切换使用既有迁移流程。秘密值仍不替代网络隔离、数据库权限及外部服务的密钥轮换。
...
Skill 支持部署目录和网页上传两种来源。目录版本由部署者在 catalog.json 中注册并配置 grant；上传版本由有写权限的租户管理员提交，平台管理员审核后仅对所属租户开放。上传要求有界的 SKILL.md，可选 run.sh，也支持受限 ZIP；压缩包不解压到任意路径，不执行上传内容。说明型不开放脚本工具，框架继续负责 Markdown 解析和 skill_load。

上传内容按租户、名称和版本保存到共享数据库，版本内容与校验值不可修改。审核更新使用 revision 乐观锁并与审计同事务提交。平台每次新运行及 skill_run 执行前重新核对批准状态；撤销后缓存 Agent 不能继续开启新执行，已经开始的沙箱进程不被强行中断。新版本需显式加入 Agent 草稿再发布，网页不能覆盖部署目录版本。
