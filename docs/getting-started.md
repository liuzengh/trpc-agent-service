...
阅读定位：这是按开发阶段展开的链路教程，前 1–10 节描述最初的单 Agent 示例，后面逐步加入平台能力；“当前示例没有实现”仅指所在阶段。第一次部署看[运行手册](operations-runbook.md)，了解最终范围看[基本交付说明](delivery.md)。原有代码拆解保留，不需要把历史实验全部重做。
...
第一阶段先不讨论多租户、消息队列、分布式锁或 Kubernetes。目标只有一个：亲手跑通下面这条链路。
