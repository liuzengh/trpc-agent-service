...
知识库支持网页配置、文本资料入库、删除、租户隔离检索和后端迁移。首次使用按以下顺序操作：

1. 准备平台节点可访问的 Qdrant 和提供 `/embeddings` 接口的模型服务。Qdrant 连接使用 gRPC 端口，通常为 `6334`；向量维度必须与 Embedding 模型输出一致。
2. 进入“资源中心 → 数据后端”，新建“知识库 / Qdrant”连接并绑定到 Agent，或设置为工作空间默认后端。保存连接只校验格式，不代表服务已经连通。
3. 进入 Agent 工作台，在“知识与记忆”开启知识库检索，填写 Embedding 模型、API 地址、向量维度和 API Key。保存草稿时，API Key 使用安装主密钥加密保存，不写入 Agent 版本，也不会在页面回显。
4. 校验并发布 Agent 版本。进入“资源中心 → 知识库”，选择已发布的 Agent，上传 UTF-8 文本、Markdown、JSON 或 CSV，也可以直接填写正文。单份资料最多 512 KiB；PDF、Word 等文件需先转换为文本。
5. 列表会显示等待入库、正在入库、可检索、正在删除或处理失败。只有状态变为“可检索”后，才在工作台新建调试会话验证 `knowledge_search` 的结果。

同一 Agent 中再次使用相同资料标识会替换原内容。删除会提交后台任务，完成后从列表移除。知识资料按租户和 Agent 隔离，不是每个 Revision 的私有副本；回滚 Agent 配置不会回滚已经导入的资料。更换 Embedding 模型或向量维度时需要重建或迁移索引，不能只修改模型名称。
...
网页存储连接与 Skill 上传需要 schema 32，网页知识资料目录需要 schema 33。迁移增加资源表、不可变版本约束和知识任务状态同步，不清空已有空间、模型、会话或凭据。分角色部署还需补齐 Admin 对新资源表的管理权限、Worker 对 Skill 表的读取权限及 Jobs 对存储凭据的读取权限；沿用已有主密钥，不重新初始化安装数据卷。
