...
Poller 在群级租约内读取完整窗口，调用 chat_messages_list 分页后排序，逐条校验并交给统一 Gateway。首次从 start_at 开始，有限重叠补偿延迟；单窗口最多 20 页、1000 条，HTTP 响应最多 2 MiB。游标缺失/重复、页级错误和超限保留检查点，不假造成功进度。
