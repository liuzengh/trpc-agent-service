...
先按[公网服务器部署步骤](operations-runbook.md#2-公网服务器部署)配置域名、Nginx 和 HTTPS，然后在“服务地址”填写平台域名。反向代理只转发回调与 `/healthz`，不公开 `/admin/`。保存地址不等于已经配置网络；地址变更用于新连接，已有机器人保持原地址。
...
群与成员授权必须逐群确认；不能将两张独立名单交叉组合为授权范围。
...
原来的服务器配置继续有效，页面标注为“服务器配置”。可以暂停、启用和查看消息记录，平台不会自动重新注册或替换其凭据。网页暂不自动接管这类绑定；同一个 Telegram Bot 已在本服务配置时，会提示使用已有连接，避免产生两份路由。
...
IM 默认采用 `message_policy={"mode":"realtime","max_age_seconds":120}`。120 秒表示企业微信的近期接收窗口，不是消息有效期：合法消息先持久保存，Telegram 延迟投递和已入队请求不会因年龄被丢弃。近期接收与历史补读独立运行，近期请求与恢复请求分队列调度。`mode=reliable` 保留按历史检查点顺序接收的方式；两种模式都不自动重放旧版已经终止/忽略的请求。

模型连接失败、HTTP 408/429/5xx 且当前请求尚无模型输出或工具执行时，请求进入 `waiting`，延迟 5/10/20/30 秒调度，之后间隔上限为 30 秒，不消耗普通执行失败的三次尝试。等待不占 Worker 执行槽或租户并发租约；同租户/应用/版本共享短暂退避。平台为每条请求最多创建一条等待提示，恢复后仍使用原请求编号产生最终回复。确定无法执行的错误仍终止并反馈，结果未知的工具/发送不盲目重试。
...
这是自建应用协议，不适用于只提供 StreamableHttp URL / JSON Config 的消息 MCP 页面。两种接入方式使用独立配置，应分别确认回调、授权与发送通路。
