...
IM 默认采用 `message_policy={"mode":"realtime","max_age_seconds":120}`：只接受近期聊天，过期消息确认接收并记录，但不触发模型、工具或旧回复。Telegram 停机后的延迟投递也适用；入队后超过期限且尚未开始的请求同样不执行。已开始执行和结果未知的任务不套用过期清理。需要完整历史处理时，须明确选择 `mode=reliable`，并接受历史积压延迟；此选项不会自动回放已忽略的消息。
...
Poller 在群级租约内读取完整窗口，调用 chat_messages_list 分页后排序，逐条校验并交给统一 Gateway。近期优先模式每次直接读取当前有效期内的窗口，同时尊重 start_at 和人工恢复的下界；旧检查点不会挡住近期查询，跨过的区间与检查点推进在同一事务中审计。只有 reliable 模式从旧检查点逐段补读。单窗口最多 20 页、1000 条，HTTP 响应最多 2 MiB；游标缺失/重复、页级错误和超限时保留检查点，不假造成功进度。
