...
2026-09-10 日常环境完成 26→27 增量升级，升级前备份数据库、角色、私有配置与旧二进制；配置指纹和 `.env` 保持不变。新版就绪和公网健康检查通过，真实模型在工作台的独立会话成功调用只读项目文档 MCP 并返回预期回复，有工具成功记录和 trace_id。此验证不是 IM 入站/出站确认；最新版本的 Telegram、企业微信完整链路仍由用户实际发消息核对，不能将旧版确认代替本轮确认。
