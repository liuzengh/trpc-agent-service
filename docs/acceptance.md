...
`0.3.0-rc.2` 针对上述停机体验补充近期消息优先。InMemory/PostgreSQL 隔离检查已验证：落后 12 小时的检查点首次只查询当前有效窗口并记录略过区间；旧 Telegram 消息确认接收但不排入 Agent，新消息携带有效期入队；未启动任务过期后不创建回复，已运行和结果未知的任务不改写为过期。当前实例已升级 schema 25，并启用默认 120 秒近期策略；原应用/版本/通道配置指纹与 `.env` 保持不变。这些验证不等于上游永不延迟，也不替代旧版本混跑兼容性审查。
