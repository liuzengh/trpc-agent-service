...
升级核对包括：基线 `0ce285f` 在 schema 24 上启动、管理查询及 HTTP Agent 执行；日常数据库备份、23→24 增量升级，升级前后租户/应用/版本/通道/后端配置指纹一致；现有真实模型通过新版网页调试任务返回预期结果。它们不等于升级后的真实 IM 发消息确认或生产验收。源码交付归档仍需从本轮提交生成并核对干净构建；P6 的最终状态以这两项后续结果为准。
