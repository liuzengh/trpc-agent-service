...
| 6 | **附件外置** | 框架 `session/externalization`（`buildRunnerOptions` 里包装会话服务） | 事件落库前把内联图片/音频/文件字节写进制品库，事件里只留 `artifact://名字@版本` 引用；**同时计入 artifact 用量维度** |
...
| 故障场景 | 真实行为 | 代码位置 |
| --- | --- | --- |
| **MySQL 不可用（启动时）** | 启动等待 MySQL/Redis 就绪（60s 轮询）后仍不可用 → `health.Report("mysql","unavailable")`，`/healthz` 返回 **503 + degraded 原因**；管理域回落内存（若配置允许），数据面不启动 | `main.go`、`infra/health` |
| **Redis 不可用（运行期）** | 消费循环与 IM 出口跟随者由 `health.Supervisor` 托管：**失败即退避重连**（1s→30s），依赖**沉默超过 5s**（总线正常时每秒都会应答一次读）就把节点标 `degraded`（`/healthz` 503），**总线重新应答读的瞬间自动清除**。出站/发布类命令各自重试，不因瞬态错误退出。实测：停 Redis 30s → t+21s 转 503、恢复后 ≤6s 回到 200、进程未重启、消息零丢失（《部署与运维手册》§2.5） | `infra/health/supervisor.go`、`bus.SetConsumeReady`、`channels.Gateway.SetReady` |
| **节点故障（worker 进程被杀）** | 未 ack 的消息留在 PEL，`XAUTOCLAIM` 认领后重投；幂等键是**租约**（90s，回合内每 10s 续期），崩溃时租约到期 → 重投会重跑该回合，而 MySQL outbox 的 `message_id` 唯一标记保证**不重复回复**；会话锁 30s + 心跳续期防止两个节点同时跑同一会话 | `bus.reclaim`、`bus.Idempotent/CommitIdem`、`worker.startTurnHeartbeat` |
| **模型超时** | 两层预算：单次模型 HTTP **120s**（`llm.DefaultHTTPTimeout`，三协议均接线）+ 整回合 **300s** 硬超时（`worker.runTimeout`，含多轮工具调用）。实测（`scripts/faults/model-timeout.ps1`）：黑洞地址属连接层失败 → ~68s 归类 **`run_error`**；「连上但不应答」的 stall 上游会被 SDK 重试放大，累计到 ~283s 由**回合级护栏**切断 → 归类 **`timeout`**（`timeoutAware` 把平台 deadline 一并包装，见 R14）。被切断的回合消息重投 | `llm.DefaultHTTPTimeout`、`worker.runTimeout`、`worker.timeoutAware` |
...
| **IM 平台重复回调** | 入站消息 ID = `channel:platformMsgID`，Redis 幂等键两段式（`SetNX` 抢占 + 成功 `CommitIdem` + 失败 `ClearIdem`）→ 同一条回调重复到达只产生一次回合、一条回复 | `channels.gateway`、`worker.handle` |
...
**灰度发布的对象是平台版本，不是 Agent 资产。** 平台侧没有"把 P% 的会话路由到另一个
Agent 版本"的能力（也不打算有，见 §10.4）；发布灰度做在**入口层**，因为平台是单一二进制
+ 角色标志，新版本就是一个新容器：

| 能力 | 实现 | 可观测证据 |
| --- | --- | --- |
| 按比例灰度 | nginx `split_clients` 按 `${cookie_dsh_uid}${remote_addr}` 稳定分桶（同一浏览器命中同一版本） | `deployments/canary/weight.conf` 由 `scripts/canary.sh weight N` 生成并 reload |
| 按租户定向灰度 | `map $http_x_tenant_id` 试点租户白名单，**优先于百分比**（白名单租户钉死走新版本） | `scripts/canary.sh tenant-check <tenant> N` |
| 灰度可验证 | 每个进程暴露 `GET /version`（`{version,git_sha,role,instance,started_at}`，无需鉴权） | `scripts/canary.sh check N` 采样后给出真实占比 |
| 一键回滚 | `canary.sh rollback` 权重置 0 并 reload；旧容器仍在，秒级生效 | 回滚后再采样应为 100% 旧版本 |
| 全量提升 | `canary.sh promote` 用新构建重建主服务并移除灰度容器 | `canary.sh status` 显示两个服务的 `/version` |

**两个边界（如实说明）**：

1. **IM 网关不能按比例灰度**：一个 bot 账号只能由一个进程持有长连接，百分比切流对它
   没有意义（两条 WSS 连接只会各自收到一半回调，幂等键挡不住"两条流各自收到不同消息"）。
   网关只能**蓝绿**：停旧节点网关 → 起新节点网关。因此灰度范围内必须区分
   **无状态面**（Admin API / Chat API / Worker 消费，可灰度）与**网关面**（只能蓝绿）。
2. **前端随平台版本整体切换**：前端是静态产物 + nginx，灰度时只对 `/api/` 做分流，
   页面本身仍是一份（API 契约向后兼容，新前端不依赖未发布的接口）。

**Agent 发布/回滚**：每次 publish 生成不可变版本行（`agent_versions`），回滚是原子切换
`agents.current_version` 指针，正在跑的回合不受影响。**Agent 版本不做分流**——同一租户的
Agent 要么全量在新版本，要么全量回滚。

**租户配置回滚**：每次更新写 `tenant_config_versions` 快照，`POST /tenants/{id}/config-rollback`
恢复；平台外另提供 `cmd/configctl`（export / diff / apply / versions / rollback），
用于**平台不可用时**或变更前留档的场景。

### 9.4 容量评估参考

| 指标            | 参考值（真实模型实测，见《部署与运维手册》§4） |
| ------------- | ---------------------------------------- |
| 单节点并发 Session | 16（`consumeWorkers` 默认值，即有界并发回合数） |
...
### 10.4 为什么不做 Agent 版本分流（决策记录）

Agent 侧**没有**"按比例/按会话把流量分到不同版本"的能力，这是有意划出范围，不是待办：

- 需求里的"灰度发布"指**平台项目版本的灰度**（新构建先接少量流量），由入口层实现（§9.3）；
- Agent 资产用**不可变版本 + 原子切换**：publish 生成 `agent_versions` 行并推进
  `agents.current_version`，回滚就是切指针。同一租户的会话要么全在新版本、要么全在旧版本，
  没有"一部分会话跑 v1、一部分跑 v2"的状态；
- 历史上实现过一版 `agents.gray`（`fnv32(sessionID)%100<P` 分桶），随后**连代码一起移除**：
  它制造了"同一 Agent 两个版本并存"的运维与审计歧义（审计要看版本、成本要按版本归集），
  而平台版本灰度已经覆盖了"新代码先小流量验证"的真实诉求。

若要重新引入 Agent 级分流，正确做法是把它做成**平台版本灰度的一种**（新版本整体接小流量），
而不是在同一进程里按会话分桶——后者会让"这个回合到底跑了哪份代码"变得不可回答。
...
**逐条对照**见 `docs/验收检查清单.md`（每条要求 → 代码位置 → 测试/实测证据）。
...
| 真实 IM 平台行为（按钮点击、附件下载、回调验签） | 代码/配置/单测就绪，**需真实账号手测** | 步骤见 `docs/部署与运维手册.md` §3\.3 与 §6\.2 |
