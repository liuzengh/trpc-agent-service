...
5. **Session / Memory 读写有独立子 span**：`storage.tracingSessions` /
   ...
   update_session_state/summary…）与 `memory.*`（read/search/add/update/delete/
   clear）逐个方法开 span，属性含 `session.app_name`/`user_id`/`id`、
   `event.request_id`、`memory.count`、`storage.domain`/`backend`，失败记
   `RecordError` + `codes.Error`。它们是 `agent.run` 的子 span（回归测试
