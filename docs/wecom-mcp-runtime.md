...
范围配置改变会触发配置指纹冲突，不会自动重置进度。只启停时保持原 config 即可续跑；配置回滚为原值可继续使用原检查点。读取范围/起点调整需要受控迁移，但当前没有检查点重置/补拉 API。不要删除旧检查点或另建绑定来绕过去重；同账号还有控制面唯一约束，正式调整流程仍待完善。
...
首版只支持群文本。白名单成员的媒体或未识别结构会让窗口报错并保留检查点，不下载、不进入模型；媒体拒绝回执需取得真实结构后补齐。私聊、卡片、文件发送和编辑语义未开放，不能把 Telegram 已支持的媒体提示算作 MCP 能力。
