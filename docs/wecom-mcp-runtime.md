...
以下步骤供新环境启用使用。既有测试群已经完成一次启用，不必重建 Binding 或重设起点；真实记录见本文开头的启用证据。新环境先确认专用测试群、允许的成员，以及是否接受指纹去重语义。
...
PostgreSQL 需按既有迁移流程应用全部迁移。`014_channel_poll.sql` 增加绑定的租户复合唯一约束及三张表；后续 015 增加异常隔离/恢复，016 增加积压视图：
...
后续真实图片/文本测试见[媒体联调记录](validation/wecom-media-2026-09-06.md)：图片未阻塞文字，但暴露了重复读取时图片 `media_id` 变化的问题。`0.2.0-rc.2` 改为稳定信封字段的媒体隔离分组，新图片经正常接收、PostgreSQL 入库和完整重叠轮询后，隔离/审计仍各 1 条，没有触发 Agent 或回复。具体合并语义见[通道恢复](channel-recovery.md)；图片内容理解仍未实现。
