...
| 知识库 | 已实现文本入库、删除、租户隔离检索和后端迁移 | 需手动配置 Embedding 密钥与授权、绑定存储，通过 Admin API 导入资料；网页仅展示绑定和配置检索参数，不提供文档上传或编辑 |
...
知识库并非只有展示页面：可按[手动配置与资料导入步骤](operations-runbook.md#knowledge-setup)使用已有后端能力。只修改环境变量不会自动创建知识库绑定或导入文档。
