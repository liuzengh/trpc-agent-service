...
**导师确认（8/21）**：「至少两种 IM」须在代码中实现（企微 + 微信客服）；Web UI（webchat 通道）作为本地自证手段与演示入口，不替代 IM 实现。接口抽象与验证矩阵详见 `docs/spec-im-channels.md`。
...
2. **可运行的最小闭环**：租户创建 → 绑定通道（webchat 本地自证；企微由导师代填真实配置验证）→ 用户对话 → Agent 调用工具 → 会话/记忆持久化 → 回复，端到端演示（IM 策略见 `docs/spec-im-channels.md`）；
