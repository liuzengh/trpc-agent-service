...
`P2-04: verified only as a local integration, bounded stress/failure, security-test and production-operations-runbook boundary`。四个子边界：A 集成/契约回归 PASS（8 链矩阵：配置启动、webhook 入站、Queue/Worker/Completion、Session/Event/Memory、Outbox/Dispatcher/Sender、config publication、Artifact/ObjectStore、Observability，各链 positive/negative/timeout/restart/租户隔离证据齐备）；B 有界压力/故障 PASS（B1-B5 由 P2-03 套件覆盖 + 新增 B6 有界 soak：120 操作/并发 6/45s deadline，accepted==durable enqueues、goroutine 高水位 400 且结束无泄漏、admission 归零、recording sender 120:0、零外部请求）；C 安全负向与审计 PASS（有保留）：新增 TestP204SecretScan（14 目标×7 类高信号模式）、TestP204ComposeSecurityAudit（无 latest/loopback-only/secret-file/无 test-only 服务于默认 profile/healthcheck）、TestP204AppContainerRuntimeAudit（运行栈 PID1 非 root/进程 env 无明文 DSN）、gitleaks/govulncheck/trivy UNAVAILABLE、既有 5 个 reachable findings 保留 → `Project release/security closure: PARTIAL`；D 运维手册/发布门禁 PASS：`docs/P2-04运行文档.md`（25 项 + 23 行事故矩阵 + 发布门禁清单）与 `scripts/p204_release_gate.sh`（6 步快速门禁，实际执行 PASS）。生产代码零改动（P0-09/P1-x/P2-01/02/03 合同保持）；migration delta 0；依赖 delta 0。执行期间远端断电一次（电池耗尽），恢复后验证树完整性并完成资源清理（containers=0/networks=3 默认/本轮 volumes=0（156 个匿名卷按创建时间清理，831 历史卷保留）/images 本轮 0/tmp=0/processes=0）。full ordinary 42 包 / full race 42 包 / integration ordinary+race 41 包 PASS（DATA RACE 未观察）；vet/gofmt/targeted diff-check PASS；compose 三门禁显式重跑 PASS；`External provider requests in P2-04: 0`。`Production integration/deployment: NOT RUN`；`Security clean: NOT CLAIMED`；`P1-06 production rollout: BLOCKED`；`Production Milvus: NOT RUN`；`Production Embedder: NOT IMPLEMENTED`；全部 production capacity/HA/autoscaling/SLO/PITR/RPO 语句保持 NOT PROVEN/NOT IMPLEMENTED。无 commit/push。

## POST-P2 readiness gate（当前状态）

`POST-P2 readiness gate: PASS — production-readiness inventory and Go/No-Go decision pack completed`。P2-01..P2-04 全部关闭后，本门在全新 owner-scoped 套件上重跑全部可重复门禁：release gate 6 步 PASS、compose config PASS、full ordinary/race 42 包 PASS、integration ordinary/race 41 包 PASS（含 compose 三门禁与 P2-01/02/03/04 focused gates）、vet PASS、targeted diff-check 0 findings——P2-04 交付可重复。生产化阻塞盘点与决策包见 `docs/POST-P2-readiness-gate.md`（P0：依赖 findings/RLS rollout/deployment/security closure；P1：HA/备份/telemetry/Secret-IAM；P2：容量/公平；P3：Vector 生产链路等；每项含工作流选项与 Go/No-Go 准则）。本门不是生产部署、不是 P3 实施阶段。

**WS-1 remediation（用户授权决策后执行）**：定向升级 7 个模块——pgx v5.7.2→v5.9.2（GO-2026-5004）、go-redis v9.7.0→v9.7.3（GO-2025-3540）、grpc v1.65.0→v1.82.1（GO-2026-6061）、x/text v0.21.0→v0.39.0（GO-2026-5970）、OTel v1.29.0→v1.43.0（GO-2026-4394）、gorilla/websocket v1.4.2→v1.5.3（GO-2026-6278）、etcd/server v3.5.5→v3.5.33（GO-2026-6114/5736）+传递升级（x/sys、genproto、protobuf）；govulncheck 复扫 **0 reachable vulnerabilities**（此前 8 项）；升级后全量回归 ordinary/race 42 包、integration ordinary/race 41 包全部 exit=0 无 DATA RACE；编译零改动。基线哈希更新为 go.mod `8587b80a…`、go.sum `223dfac9…`；WS-2..WS-8 选项 A 路径采纳为正式决策，执行待生产基础设施。`Security clean: NOT CLAIMED` 保持（govulncheck 修复≠完整 SAST/生产运行时审计）。

## POST-P2 执行记录：VM1 生产形 rollout（WS-2/WS-3A/WS-4 首步）

用户授权决策并提供基础设施（物理机 + 2 台 VM）后，POST-P2 决策包的 WS-3A/WS-2/WS-4 首步已在 VM1（Ubuntu 20.04.6，192.168.1.6，2C/3.9G/95G）执行完毕：Docker 28.1.1 + Compose v2.35.1 安装；`deploy/production/vm1.compose.yml` + 幂等 provisioning 脚本（`scripts/p2-prod-vm1-provision.sh`，repo 持有）落地 `/opt/p2prod`（secrets 0700/0600 随机密码、redis.conf 由 secret 生成、pg-init 创建 `repl` 复制角色并预备 pg_hba——为 VM2 standby 预备）；app 镜像经 save/load 交付（VM1 无需 Go 工具链）；`trpc-migrate` 从物理机执行 `migration ok current_version=11` + `trpc_runtime`（NOBYPASSRLS）供给。**WS-2 RLS rollout 审计**：P2-01 五项套件（policy coverage/双租户隔离/连接复用无泄漏/并发租户/窄全局能力）对 VM1 PG 全部 PASS。**WS-4 备份离机**：`trpc-recovery backup`（recovery 工具镜像）→ 归档落物理机（离机副本，sha256 `4817491f…`）+ verify 24/24 PASS + restore 演练至 scratch 库 PASS（11 migrations，演练后库删除）。应用 healthy：`/healthz=200`、`/livez=200`（VM1:8080）。MODEL_PROVIDER=runner：外部请求 0。边界保持：`Production deployment: NOT RUN`、`Production RLS role/database rollout: NOT RUN`、`Automatic PG failover: NOT PROVEN`（2 节点手册化 promote，自动 failover 待 VM2/仲裁评估）、`Production telemetry rollout: NOT RUN`、`Security clean: NOT CLAIMED`。VM1 弱密码（123456）已建议轮换（key-auth 已建）。VM2 待用户克隆 VM1 后按 `docs/POST-P2-vm1-production-rollout.md` 第 8 节接线。无 commit/push。
