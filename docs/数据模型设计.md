...
| `runtime_profile` | JSON NOT NULL | 冻结的运行时配置，字段名与 `domain/agent.RuntimeProfile` 的 json tag 一致：`system_prompt`、`endpoint_id`、`tool_ids[]`、`kb_ids[]`、`skill_ids[]`、`approval_tool_ids[]` |
...
| `stream:outbound` | Stream | 无 | 出站回复流（游标跟随，无消费组） |
| `stream:inbound:dlq` | Stream | 无 | 死信队列流（另有 `dead_letters` 表） |
| `idem:{msgID}` | String | 租约 90s / 提交后 24h | 两阶段幂等标记（`msgID = channel:platformMsgID`） |
| `lock:session:{tenant}:{session}` | String | 30s（心跳续期） | 会话分布式锁（Lua compare-and-delete 释放） |
| `route:{tenant}:{session}` | String | 7d | 会话 → Agent 路由 |
| `imroute:{session}` | String | 30d | 会话 → (binding, chat_id) 回复路由（网关重启后仍能应答） |
| `cursor:outbound` | String | 无 | 网关出站游标（重启不丢停机期回复） |
| `approval:req:{tenant}:{session}` | String(JSON) | 5min+ | 待审批请求（单会话单 pending） |
| `approval:res:{tenant}:{session}` | String | 2min | 审批结果（approve/deny） |
| `retry:{streamEntryID}` | String | 24h | 入站失败计数（达 `DefaultMaxDeliveries=5` 进 DLQ） |
| `ratelimit:{tenant}:{channel}:{user}` | String | 窗口 | 入站限流计数（默认关闭） |
