...
CREATE TABLE gateway_authorization_principals (
 scope_id text NOT NULL,
 account_id text NOT NULL,
 ...
 FOREIGN KEY(scope_id,account_id,generation,tenant_id,provider)
   REFERENCES gateway_authorization_snapshots(scope_id,account_id,generation,tenant_id,provider)
   DEFERRABLE INITIALLY DEFERRED
);
CREATE FUNCTION gateway_authorization_snapshot_guard() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
 ...
 RETURN NEW;
END $$;
CREATE TRIGGER gateway_authorization_snapshot_guard BEFORE UPDATE OR DELETE ON gateway_authorization_snapshots
 FOR EACH ROW EXECUTE FUNCTION gateway_authorization_snapshot_guard();

-- Staging uses normal DML, not CREATE TEMP TABLE: gateway_runtime intentionally
...
CREATE TABLE gateway_authorization_staging (
