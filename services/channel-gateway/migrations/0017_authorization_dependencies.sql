...
ALTER TABLE gateway_authorization_snapshots
 ADD COLUMN session_policy_jsonb jsonb,
 ADD COLUMN quota_policy_jsonb jsonb,
 ADD CONSTRAINT gateway_authorization_dependencies_complete CHECK (
...
CREATE FUNCTION gateway_authorization_dependencies_guard() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
 ...
 RETURN NEW;
END $$;
CREATE TRIGGER gateway_authorization_dependencies_guard BEFORE UPDATE ON gateway_authorization_snapshots
 FOR EACH ROW EXECUTE FUNCTION gateway_authorization_dependencies_guard();
